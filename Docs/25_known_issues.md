...
| §3-2 | プロフィール表示が生 enum(`short_walk_ok / relaxed`) | **解消(2026-08-05)**: `lib/profileLabels.js` の対応表で日本語化。**表にない値は落とす**(生値を出さない)。enum は [agent_react_architecture.md §14](30_design/agent_react_architecture.md) とバックエンド実コード 4 箇所に照合済み。実機で「家族(子連れ) / 軽い徒歩ならOK / のんびり」を確認 |
| §3-4 | スポット一覧に内部タイプ `（tourist_spot）` が出る | **解消(2026-08-05)**: `poiListLabel` からカテゴリを除去。実機で旅程ストリップに内部語なしを確認 |
| §3-6 | 同じ通知が毎ターン重複して積み上がる | 要再確認(今回は差分のあるターンだけ通知が出ており、悪化はしていない) |
| §4-1〜4-9 | レイアウト(スターターカードの重なり・地図パネルの操作性・アニメーション等) | **大半が解消(2026-08-05、[ADR-0023](adr/0023-frontend-design-system.md) の表示層作り直し)**。個別の状況は下表 |
| §5-1〜5-4 | 候補カード・旅程行が押せない、本文とカードの二重表示 | カードの操作性は**未対応**(機能追加のため [ADR-0023](adr/0023-frontend-design-system.md) のスコープ外。[90_backlog.md](90_backlog.md))。二重表示は [dialogue_style.md](30_design/dialogue_style.md) の新様式で実質軽減 |
| §5-5 | リロードで候補カードの説明文が消える | 要再確認(`meta.presented` の整備で改善した可能性) |
| §5-6 | リロードでウェルカムメッセージが消える | **解消(2026-08-05)**: 自動挿入のウェルカムメッセージ自体を廃止し、**空状態の見出しと提案ピル**に置き換えた(消えるものが無くなった) |
| §5-7 | パック ETA | 未対応 |
| §6-1〜6-4 | 言語・表記(ヘッダー "AI Agent by Qwen3"、ログイン画面英語、地図まわりの英語ラベル、死んだ "Sign up" リンク) | **全件解消(2026-08-05)**: ヘッダーは実際の生成モデル `Gemma 4` へ。ログイン画面を日本語化し、**登録/ログインのモード切替ごと廃止**(`POST /login` はユーザー名のみで 2 モードの必然性が無い → 死んだ `a[href="#"]` も消滅)。地図まわりも日本語化 |

### 3.1 表示層の作り直し(2026-08-05)で §4 がどうなったか

**根拠**: [ADR-0023](adr/0023-frontend-design-system.md) / [frontend_design_system.md](30_design/frontend_design_system.md)。実装 = Sonnet 5、レビュー = Opus 5、受け入れ = Fable 5(実機実測)。

| 旧 23 の # | 内容 | 現状 |
| --- | --- | --- |
| §4-1 | スターターカードが会話に重なる(`absolute` で無条件描画) | **解消**。提案は**空状態にしか出さない**(会話が 1 件でもあれば DOM に不在)。フローティングカード 3 枚は削除 |
| §4-2 | 地図が会話の真上に開く | **仕様として維持**。ユーザー判断(2026-08-05)で**フローティング窓の形は残す**と決定。開閉はヘッダーのピルに移し、**トグルが本文に重なることは構造的に無くなった** |
| §4-3 | 地図がコントロールで埋まる(420px に 4 枚が重なり地図は右端の細い帯) | **解消**。地図上の常設要素は**現在地追従ボタン 1 つだけ**。取り込み・オフライン資材・ライブ同期・LoRa はヘッダーの ⋯ メニューへ。旧 "Spots List" は廃止し、**窓の下端に旅程ストリップ**を新設(地図と旅程を同時に読める) |
| §4-4 | 地図トグルが `pulse-glow 2.5s infinite` で自走し、自動操作でクリックできない | **解消**。無限アニメーションを廃止(旅程更新時に 1 回だけ脈打って止まる)。**Playwright の通常 `click` が成立**することを受け入れ時に実測 |
| §4-5 | モバイルでトグルが吹き出しに重なる | **解消**。トグルを sticky ヘッダー内に移動(本文領域と別 flow)。※当初「右下に浮かせる」で設計したが、モックアップ検証で発話に重なることが判明し、**浮かせる案そのものを捨てた** |
| §4-6 | モバイルでスターターカードが見切れる | **解消**(§4-1 と同じ構造変更)。844×390 で 3 枚ともクリック可能・スクロール可能を実測 |
| §4-7 | モバイルで地図パネルがほぼ全画面なのに全画面表示ではない | **未対応**(窓の寸法方針は変えていない) |
| §4-8 | 小さいビューポートでログイン画面のタイトルとカードが重なる | **解消**。旧実装の `absolute` + `mt-32` の力技をやめ、**タイトルとパネルを縦に積んで中央寄せする**組み方にした(重なりが原理的に起きない)。**780×493 実測: タイトル底 144 / パネル上端 192 で交差なし、`scrollHeight 584 > clientHeight 493` でスクロール到達可**([frontend_design_system.md §10](30_design/frontend_design_system.md) 条件 19) |
| §4-9 | 「準備完了」後もパック生成のタイマーが動き続ける | **未対応**(表示層でなく状態管理) |
...
- **(2026-08-05) Playwright(headless Chrome)から `POST /api/v1/login` が 400 になり、ブラウザ自動操作でログインできなくなることがある。**アプリのコード起因ではない。切り分けの根拠:
  - **アプリのコードを通さない素の `fetch()`(ページ内から直接)でも同じ 400** が返る
  - **同じ Vite プロキシ経由の `curl` は 200**(`POST http://localhost:5173/api/v1/login`)
  - **バックエンドのログに該当リクエストが無い**(`request_completed` は 200 のみ)
