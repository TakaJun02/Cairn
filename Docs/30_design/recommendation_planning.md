...
| 3 | FR-1 の既定構成 | **案 C**(コードが候補 8〜12 件を作る → LLM が最終 **3 件**を選び理由を書く。**2026-08-04 ユーザー決定で件数を 3 に固定** — 旧 3〜5 件。[dialogue_style.md](dialogue_style.md) で全件を本文で語るため)。案 B(コードのみ)は比較用に flag で残す | §3.2 / §3.3 |
...
| 8 | 対象範囲 | **鳥海山エリア限定**(全国展開しない。全国の自然公園 POI 収集は現実的でない)。対外発表には使わない(発表済み) | §0.1 |
| 9 | 説明の忠実性 | 深追いはしない。**ただしクローズドワールドと根拠素材の注入は品質のために維持する** | §3.3 |

### 0.1 「対外発表に使わない」ことの設計上の含意

ユーザー確認(2026-07-30): **本システムは対外発表には用いない**(発表は既に完了しており、以降は個人の作り込み)。**対象エリアは鳥海山エリアに固定**する。これにより §2 の調査結果は「品質を上げるための根拠」としてのみ効き、以下が不要になる:
...
| 規模・目的 | 開発者 1 名・同時利用〜数十・個人開発プロダクト。変更容易性(NFR-1) | 学習ベース推薦に必要なインタラクションデータは**構造的に集まらない**(コールドスタートが常態) |
...
- ChatGPT Memory(製品): 保存メモリをユーザーが閲覧・削除・無効化できる。本件でも**プロファイルを可視化・編集可能にする**ことは、透明性・スクラタビリティの面で踏襲価値が高い
...
- LLM による推薦説明を直接扱う文献はまだ 6 件しか特定されていない(Frontiers in Big Data 2024 のレビュー、232 件をスクリーニング)= **確立手法がなく、自前の忠実性担保設計が必要な領域**
...
**(d) 日本語圏の学術動向(本件に近い先行事例)**

- **観光情報学会 第21回全国大会**(2025-06, 京都橘大学): 「**対話入力可能なパーソナル観光旅程推薦システム**」(東京大・北海道大)= **本件と同型の先行事例が既にある**。ほか「LLM を用いたオーバーツーリズム緩和のための代替観光地提案システム」(北海道情報大)、観光レビューの暗黙的感情推定(北見工大)など
...
| **旅程差分の提示と undo** | **主機能として打ち出す実プロダクトは確認できず** | AI による変更は「+2 件 / −1 件 / 順序変更」の差分サマリ + [元に戻す] を必ず添える。**本システムの差別化点になりうる** |
...
```jsonc
{
  "interests": {"nature": 0.8, "water": 0.6, "shrine_temple": -0.3},  // キーは PreferenceKey 12 語（data_model.md §2）。重み -1..1
...
- **プロファイルの可視化と編集**: 把握済みプロファイルをチャット上部に編集可能なチップ列として常時表示する(ChatGPT Memory / Kayak / Mindtrip、§2.3(e))。透明性・スクラタビリティの面で価値が高い
...
なお **B(リランク OFF)は flag として残す**。比較実験のためではなく、**LLM 側が壊れた/遅いときに推薦機能を生かしたまま縮退させる経路**として実務的な価値があるため(NFR-1)。
...
- **所要時間の自然文は、接地された起点があるときだけ作る**(2026-08-04 追加、[25 §1-4](../25_known_issues.md) の是正): 起点として使ってよいのは (a) 既存旅程の起点、(b) ユーザーが発話で指定した地点、のいずれかだけ。**どちらも無いときに既定の起点を勝手に選ばない**(旧実装は facility 種別のソート順先頭 = 鳥海高原家族旅行村を暗黙に採っており、未確認の起点基準の「車で約 25 分」がカードに出ていた)。起点が無ければ `reason_materials` に所要時間を含めず、カードにも表示しない。スコアの移動時間ペナルティも同様に起点があるときだけ効かせる(現行実装の None 許容のまま)。**なお現時点の実装では (b) の供給口は旅程経由のみ**(ユーザーが起点を伝えると旅程作成時に `days[].origin` になり、以後 (a) として効く)。`recommend` に起点を直接渡す引数は設けていない — 旅程がまだ無い段階の推薦カードには所要時間が出ない、が仕様である(必要が出たら引数追加を検討: [90_backlog](../90_backlog.md))
...
- **説明の忠実性は「素材の接地」までで止める**(決定 2026-07-30、論点 9): 「理由の骨格をコードが決め、LLM は言語化のみ」という**構成的忠実性の作り込みは行わない**(深追いしないため)。維持するのは**クローズドワールドと根拠素材(一致タグ・所要時間の自然文・天気適合・混雑状況)の注入**まで。これは**嘘の理由が出るのを防ぐ品質要件**だから残す。含意判定による自動検証(§2.1(e))も導入しない
...
3 経路とも設定で ON/OFF を切り替えられるようにする(**2026-08-01 改訂**: 旧「`turn_metrics` に記録して比較実験ができるようにする」。NFR-7 の削除に伴い、比較実験のためのデータ収集は行わない)。
...
- データモデル案(**当時の草案。確定形は [data_model.md §4.5](data_model.md) が正** — 時刻は `arrive_min` などの整数になり、`constraints` は旅程行の別列に移った):
...
**決定(2026-07-30、論点 2)。** 案 C を主方式とする。**当初案にあった「A(純 LLM)を ablation 用の flag として残す」は撤回**する — 対外発表に使わないと決まった(§0.1)ため比較実験の需要がなく、実装を残す理由がない。**スキーマ外の要望への対処は §4.4 で本設計に組み込む**(論点 2 のユーザー要望「スキーマにない要望も組み込めるような設計を考えたい」への回答)。
...
- **天気による再計画**: 影響を受ける POI のスコアを 0 にして最小変更ペナルティつきで再ソルブするだけで済み、**LLM 呼び出しは通知文の 1 回のみ**。発展として挙げていた「パック生成時に**雨天代替行程を事前計算して同梱**する案」(オフライン時の plan-B)は、**2026-08-01 に「行わない」と決定した**([packs_pipeline.md §2](packs_pipeline.md))。FR-4.3 が要求するのは案内内容の変化であって行程の差し替えではなく、代替行程を焼くと生成量が倍増し、端末 UI(Phase 4)に強く依存するため。**代わりに雨の overlay が「近くの雨に強い場所」を 1 件だけ挙げる**。なお**計画フェーズでの天気による再ソルブは従来どおり行う**(ここで否定したのは事前計算の同梱だけである)
...
- **メッセージは表示名で組む**(2026-08-04 追加、[25 §1-7](../25_known_issues.md)): `target`/`a`/`b` が `spot_id` のときは `name_ja` へ解決してから文面に埋める(タグ名はそのまま)。このメッセージ(`Concession.message_ja`)は LLM のコンテキスト(旅程ダイジェスト)とフロントの譲歩カードの両方へそのまま流れるため、**`spot_id` を含めてはならない**。そのために評価コンテキスト(`predicate_context`)へ渡すスポット静的情報に `name_ja` を含める。`name_ja` が引けない場合も id へフォールバックせず中立表記(「指定の場所」)を使う
...
#### (7) 編集ターンの既定挙動 — 訪問集合の固定と `allow_refill`(2026-08-04 追加、[ADR-0021](../adr/0021-edit-turn-default-lock.md))

実機で「三崎公園は外して」に対し**牛渡川も外れ、奈曽の白滝・金峰神社が入る**入れ替えが起きた([25 §1-2](../25_known_issues.md))。β × 編集距離(β=0.6)は効用 1 件分より小さく、削除で空いた時間への再充填が容易に勝つためである。上の (2) の第 3 項だけでは編集の安定性を担保できないと実測で確認されたので、**編集ターンの既定を「集合固定」に改める**:

- **`edit_itinerary` の既定では、訪問集合の変更は ops によるものだけ。**ops 適用後に残る訪問は削除保護(実装は削除保護専用の `removal_protected_spot_ids`。既存の `protected_spot_ids` は並び替えまで固定する意味を持つため使わない — [ADR-0021](../adr/0021-edit-turn-default-lock.md) 実装注記)し、**新規スポットの挿入候補を空にする**。ソルバーは並び・時刻の再調整と経路の引き直しだけを行う(物理的に時間割が組めない場合の実行可能化フォールバック削除だけは例外。diff に現れる)
- 新しい制約が集合を変えないと満たせないときは、勝手にスポットを落とさず**譲歩(Concession)として報告**する。respond が「◯◯を外せば守れます」と提案し、ユーザーが決める
- **`allow_refill: bool`(既定 false)**をメインエージェントが立てたときだけ従来どおりフル ILS(β も従来どおり)。立てるのは「代わりにどこか入れて」「空いた時間に何か足して」のような**明示要求があるときだけ**
- 集合固定のときは解が並びと時刻しか変わらないため、**解 A/B/C の生成と LLM 選択(§4.4 経路 3)を省略して単一解を返す**(編集ターンの LLM 呼び出しが 1 回減る)
- `plan_itinerary`(新規作成)は従来どおりフル ILS。上の (6) の図は `allow_refill: true` の編集にそのまま当てはまる。**この項目は (8) で改める(2026-08-04 夜)**

#### (8) 新規作成の挿入プールを明示候補に絞る — `candidate_spots`(2026-08-04 夜追加、[ADR-0022](../adr/0022-plan-turn-explicit-candidate-pool.md))

実機で「〇〇に行きたいです」と 1 スポットのみ指名したのに、複数スポットが組み込まれたプランが返る事象が確認された。原因はデータ分離バグではなく(調査で否定済み。[ADR-0022](../adr/0022-plan-turn-explicit-candidate-pool.md) 文脈参照)、上の (7) 末尾に書いた「`plan_itinerary` は従来どおりフル ILS」という 2026-08-04 昼の判断そのものだった。ソルバーは `insertion_pool=None`(制限なし)で呼ばれ、鳥海山エリア共通の観光地マスタ(`static.spots` 全 43 件)から効用最大の挿入を時間予算まで自動で続ける。ユーザーから見れば「頼んでいないスポットが勝手に入る」— (7) が編集ターンについて是正した問題と構造的に同型である。

**この判断を覆し、`plan_itinerary` にも明示候補プールを導入する**:

- `plan_itinerary` の引数に **`candidate_spots: string[]`(既定 `[]`)**を追加する。`must_visit` と同じ名寄せ経路で `spot_id` に解決する
- `SolverInput.insertion_pool` は **`frozenset(must_visit の解決済み spot_id) | frozenset(candidate_spots の解決済み spot_id)`** を渡す(`None` を渡さない = カタログ全件を暗黙に使わない)
- `candidate_spots` は `require` 制約を作らない。ソルバーはこの集合の中からだけ、時間予算・効用に応じて**入れても入れなくてもよい**候補として貪欲挿入・ILS の対象にする。**例外は require 制約(§4.4 経路 1)による明示挿入**で、`_insert_requirements` は `insertion_pool` の制限対象外(「ユーザーが明示した場所は必ず入る」。ADR-0021 実装時からの仕様)
- **空判定は実効プールで行う**(2026-08-04 夜、レビュー是正): 解決済み `must_visit ∪ candidate_spots` から**日の起終点を引いた集合**が空なら、旅程計画 SA は `ToolError(precondition_unmet)` を返す(空の旅程を黙って返さない。起点未解決 [25 §1-4] と同型のガードレール)。名寄せ全滅時のメッセージには落ちた要素・曖昧だった要素を含める
- **解 C(ゆったり版)は `required_spot_ids` を除去しない**(2026-08-04 夜、レビュー是正)。また**任意候補がゼロ(`insertion_pool ⊆ required_spot_ids`)のときは単一解とし、解 A/B/C と選択 LLM を省略する**(編集の集合固定 [ADR-0021] と対称。LLM 1 回減)
- 「おすすめのスポットで旅程を組んで」のような要求では、**メインエージェントが先に `recommend` を呼び**、結果の候補名を `candidate_spots` に転記して `plan_itinerary` を呼ぶ。専用の直結経路は作らず、メインループの通常の反復(§5 の act ループ)で表現する

**帰結**: 「〇〇に行きたい」だけの依頼は `candidate_spots` を渡さない限り〇〇だけの旅程になる(空き時間は空いたまま)。埋めたい場合はユーザーの要望かメインエージェントの判断で `candidate_spots` を明示的に渡す。ソルバー自体は「渡された集合の中で最適化する」という既存のロジックのまま変わらない。詳細と検討した代替案は [ADR-0022](../adr/0022-plan-turn-explicit-candidate-pool.md) を参照。
...
persist
  └ プロファイル・旅程(version+1)・メッセージ・スレッド状態を 1 トランザクション
```
...
> **2026-07-30 の決定により大幅に簡素化した。** 対外発表に使わない(§0.1)ため、**論文の実験としての評価設計は削除**した(人手評価のリリース閾値・10〜20 名のユーザースタディ・ResQue 系の構成概念・LLM-as-a-judge のメタ評価・ablation のための方式併存)。残すのは**開発中に壊れを検知できるもの**だけである。
>
> **2026-08-01 さらに改訂: 「層 0 システム計測」を削除した。**NFR-7(計測可能性)を要求から外し、`turn_metrics` / `unmodeled_log` テーブルと `export-metrics` CLI を廃止したため。遅い・おかしいと感じたときは**構造化ログ**([20_architecture.md §12](../20_architecture.md))を見る。旅程スナップショットは version 管理として残るが、これは **undo のための機能であって計測ではない**。
...
論点 7(データ拡充)は §6、論点 8(対象範囲と対外発表)は §0.1 に反映済みで、ADR は起こさない(アーキテクチャ上の分岐ではないため)。

**追記(2026-08-04)**: 実機確認([25_known_issues.md](../25_known_issues.md))を受けて 2 件を追加決定した。

- 編集ターンは既定で訪問集合を固定し、再充填は `allow_refill` の明示要求時だけ(§4.5(7)、[ADR-0021](../adr/0021-edit-turn-default-lock.md))
- 所要時間の自然文は接地された起点があるときだけ作る。既定起点の暗黙選択を廃止(§3.3)
...
4. **述語の初期セット**(§4.4 経路 1): 表に挙げた **17 種類**(表の行は分類 11 個だが、述語は 17 個ある)のうち、どこから実装するか。`weight` / `require` / `exclude` / `last` / `not_consecutive` / `time_window` / `lunch_break` があれば実用になる見込み。残りは `unmodeled` ログを見て足す
