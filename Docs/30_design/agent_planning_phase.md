...
- 状態: **廃止(2026-08-04。[agent_react_architecture.md](agent_react_architecture.md) と [ADR-0019](../adr/0019-react-main-agent-subagents.md) が置き換え)。**決定の経緯の記録として凍結する。**実装の参照先にしないこと**(旧状態: Part I = 承認 + 2026-07-31 改訂 / Part II = 決定稿)
...
| P5 | **`ask_user` は末尾にしか置けない。plan 全体で 1 手まで**(G1) | 末尾以外の / 2 つ目以降の `ask_user` を破棄。**質問したらターンは中断する**ので後続の手は動かない。この制約により**復帰点が常に `understand` に定まる**([ADR-0018](../adr/0018-ask-user-resumable-tool.md) / §3.4.2) |
...
| `ask_user` | `kind`(`preference`/`clarify`)、`slot?`、`surface?`、`reason`、`options` | **ユーザーの答え**(ターンを中断して次ターンで返る) | なし | FR-1.1 |
...
- **`ask_user` だけは副作用が「ターンを中断する」**という特殊な道具である。したがって P5(末尾のみ)とガードレール(§5.1)が二重にかかる
- **`ask_user` は 1 つの Tool で 2 用途を担う**(2026-08-03 改訂。[ADR-0018](../adr/0018-ask-user-resumable-tool.md))。**`kind: "preference"`** が選好の聞き取り(「どなたと行かれますか」)、**`kind: "clarify"`** が発話の聞き返し(「『2 番目』はどちらですか」)。**旧来「聞き返しは Tool ではなく `understand` ノードの分岐で行う」としていた設計(ADR-0010)は廃止された**
- **`ask_user` は結果を返す。**返り値はユーザーの答えで、**次のターンの `tool_results` としてコンテキストに入る**(§3.4.2)。「呼んで終わり」の道具ではない
...
> **2026-08-03 改訂([ADR-0018](../adr/0018-ask-user-resumable-tool.md)): `action` と `clarify` を削除した。**旧版は `action: "done" | "ask_user"` の 2 択と、`ask_user` のときだけ埋める `clarify` オブジェクトを持っていた。**聞き返しが `ask_user` Tool に統合されたため、`plan` に 1 手載せるだけで表現できる。**

**`plan` が抽出フィールドより後ろにあるのは意図的である。**「何をするか」は**抽出をやってみて初めて分かる**ので、先に判断させない(§3.1.1)。
...
## 3.4 解けないときは `ask_user` Tool を 1 手だけ載せる(2026-08-03 全面改訂)

**決定([ADR-0018](../adr/0018-ask-user-resumable-tool.md)、ユーザー指示): `ask_user` は「結果を返す 1 つの Tool」である。`understand` は聞き返したいとき、`plan` に `ask_user` を 1 手だけ載せる。**

> **2026-08-03 改訂。**旧版は [ADR-0010](../adr/0010-understand-bounded-agent.md) に基づき、`understand` に `done` / `ask_user` の 2 択を持たせ、`ask_user` を選んだら `validate_plan`/`act` を飛ばして `respond` へ抜ける(辺 E6)設計だった。**この 2 系統は統合され、E6 は削除された。**

```mermaid
flowchart TD
  A["文脈（TurnState）<br/>※ tool_results があれば載る"] --> B["understand<br/>LLM 1 回・guided JSON"]
  B --> C["抽出フィールドを埋める<br/>references / 選好 / 制約 / unmodeled"]
  C --> D{"解けたか"}
  D -->|"解けた"| E["intent + plan を出す<br/>plan: [recommend, ...]"]
  D -->|"解けない"| F["plan: [ask_user] を出す<br/>1 手だけ"]
  E --> G["validate_plan → act"]
  F --> G
  G --> H{"ask_user が<br/>実行されたか"}
  H -->|"いいえ"| I["通常どおり respond へ"]
  H -->|"はい"| J["ターンを中断<br/>state: ask_user / clarify を送出<br/>pending_ask を保存 → respond（質問文）"]
  J -.->|"次のターン：答えを<br/>tool_results に入れて"| A
```

**要点は 3 つ。**

1. **`ask_user` は 1 つの Tool である。**選好の聞き取り(「どなたと行かれますか」)も、発話の聞き返し(「『2 番目』はどちらですか」)も、**同じ Tool の `kind` 引数**で表す
2. **Tool は結果を返す。**返り値はユーザーの答えで、**次のターンの `tool_results` としてコンテキストに入り、`understand` のプロンプトに載る**
3. **ターンをまたぐのは「中断」であって「終了」ではない。**人間の答えはターンを終えないと得られないので、中断点は必ずターン境界に来る

### 3.4.1 なぜ 2 系統を統合したのか

旧設計([ADR-0010](../adr/0010-understand-bounded-agent.md))は「聞き返しを Tool にはできない」と判断していた。その根拠は**「参照が解けていないまま前段の手を組み立てさせるのは矛盾している」**だった。

**この根拠は成立していなかった。****そのときの plan は `[ask_user]` の 1 手だけでよい。**前段の手を書かせる必要はどこにもない。

| | 旧: ノードの分岐(ADR-0010) | **新: Tool 1 本【採用】** |
| --- | --- | --- |
| 参照が解けないときの plan | plan を出さない(特別扱い) | **`[ask_user]` の 1 手**(通常の plan) |
| 機構の数 | **2 つ**(Tool + ノード内分岐) | **1 つ** |
| ガードレール | G1〜G5 と G6〜G9 の**2 組** | **1 組に統合**(§5.1) |
| グラフの辺 | **E6 を足す**(understand → respond) | **E6 は不要**(通常経路を通る) |
| `respond` のモード | **4 つ** | **3 つ** |
| Tool の返り値 | **無い**(`out = —`) | **ある**(答えが返る) |

### 3.4.2 中断と復帰

**保存するのは `pending_ask` だけ**である。中断した plan も中間結果も保存しない([ADR-0004](../adr/0004-conversation-pipeline.md) の「状態は毎ターン DB から再構築する」を崩さないため)。

| 相 | 何が起きるか |
| --- | --- |
| **中断** | `act` が `ask_user` を実行 → `state:ask_user` または `state:clarify` を送出 → `should_end_turn` → `respond` が質問文を書く → `persist` が **`pending_ask`** を保存 → `done` |
| **復帰** | 次の `POST /chat` で `load_context` が `pending_ask` を見つける → 答え(チップの `resolves` または自由入力)を **`ask_user` の結果**に組み立て `tool_results` に入れる → **`understand` から再開** |

**復帰点が常に `understand` になる理由。**`ask_user` は **P5 により plan の末尾にしか置けない**(§1.3)ので、**その後に残っている手が構造的に存在しない。**したがって `act` を途中から再開する必要が生じない。そして「答えを受けて次に何をするか」を決めるのは `understand` である(`act` は渡された手を実行するだけで、答えの意味を解釈しない)。

### 3.4.3 ADR-0008(ReAct を採らない)と矛盾しないか
...
| --- | --- |
| 1 ターンの LLM 呼び出しが 3〜6 回と可変 | **`understand` 1 + `respond` 1 のまま。**中断しても復帰しても増えない |
| レイテンシが読めない | **ターン内にループがない。**中断は必ずターン境界で起きる |
| 多段の自律判断が壊れる | plan は最大 3 手のまま。多段推論を要求していない |
| 何が起きたのか追いにくくなる | **`plan` に `ask_user` があるかを見れば「聞いたターンか」が分かる**(§10) |

### 3.4.4 得られるもの

- **静かな取り違えが減る。**§3.3 が警戒していた「序数の取り違えで違う POI が旅程に入る」に、**破棄して謝る以外の対処**ができる(この利点は ADR-0010 から引き継ぐ)
- **答えが元の文脈に戻る。**旧設計では答えが「新しい発話」として入り、質問を発した時点の文脈は捨てられていた。**`tool_results` として戻せば、`understand` は「何を聞いて、何と答えられたか」を見た状態で走れる**
- **ユーザーの手間が減る。**全文を打ち直すより、チップを 1 タップするほうが速い([recommendation_planning.md §2.3(c)](recommendation_planning.md))
- **機構が 1 つになる。**ガードレール 1 組・respond 3 モード・E6 なし

**代償は「聞きすぎ」のリスク**である。これは §5.1 のガードレールで縛る。加えて **§7 の会話履歴が第一防衛線として効く** — 曖昧さの多くは直近 3 ターンの生テキストがあれば消えるので、`ask_user` まで到達しない。
...
| `ask_streak` | スレッド行 | スレッド | ガードレール G3(**旧 `clarify_streak` を統合**) |
| **`pending_ask`** | スレッド行 | **1 ターン限り**(必ず失効) | **中断した `ask_user` の問い**(`kind` / `slot`\|`surface` / `options`)。次ターンで**答えを Tool の結果に組み立てる**(§3.4.2)。**2026-08-03: 旧 `pending_clarification` を置き換え** |
...
| §3.4 | G6 選択肢を 2〜4 個列挙できない曖昧さでは聞かない / G7 同じ曖昧さを 2 回聞かない / G9 plan を出せるなら聞かない |

### 5.1 聞くか、仮定して見せるか(2026-08-03 改訂)

`understand` は、発話が理解できないとき **`plan` に `ask_user` を 1 手だけ載せて聞ける**(§3.4 / [ADR-0018](../adr/0018-ask-user-resumable-tool.md))。ただし「分からなければ聞く」にすると対話が確認だらけになるので、**いつ聞くか**をコードで縛る。
...
**ガードレール(コードで強制する。2026-08-03 に 1 組へ統合)**

`ask_user` が 1 つの Tool になったので、**ガードレールも 1 組**にする([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。**適用は `validate_plan` / `act` の層**で行う(旧版は G1〜G5 を `act`、G6〜G9 を `understand` に分けて置いていた)。

| # | 規則 | 適用 `kind` | 実装 |
| --- | --- | --- | --- |
| **G1** | **1 ターン 1 問** | 両方 | plan に `ask_user` は 1 手まで(P5)。2 つ目以降を破棄 |
| **G2** | **同じスロットを 2 回聞かない** | `preference` | `asked_slots` をスレッド状態に持つ |
| **G3** | **連続する `ask_user` は 2 ターンまで** | 両方 | `ask_streak` が 2 に達したら破棄し、**最も確からしい解釈を採って仮定を明示する**(旧 G8 を吸収) |
| **G4** | **推薦要求に質問だけを返さない** | `preference` | 手ぶらで返さない。**`clarify` には効かせない** — 何を指しているか分からない状態で推薦を返すほうが有害だから |
| **G5** | **選択肢は 2〜4 個。自由入力も受ける** | 両方 | `options` の個数を検査。UI 側は入力欄を塞がない(FR-3.2) |
| **G6** | **選択肢を「具体的に」解決できないなら聞かない** | 主に `clarify` | `options[].value` が `spot_id` か既定の解釈 enum に解決できることを検査。できなければ**その手を破棄**し、従来どおり `respond` に伝える |
| **G7** | **同じ曖昧さを 2 回聞かない** | `clarify` | `resolved_ambiguities` をスレッド状態に持ち、既出の表層形を破棄 |
| **G9** | **妥当な plan を出せるなら聞かない** | 両方 | 曖昧さが**実際に plan の生成を妨げている**ときだけ許す。「念のため確認」を禁止する |

> **旧 G8(連続する聞き返しは 1 ターンまで)は G3 に吸収した。**両 `kind` に同じ上限がかかるほうが、「聞きすぎ」の抑制として素直である。**G4 だけは `kind` で効き方が違う**ので上表に明記した。

**聞いたあと、元の要求が復元されなければ意味がない。**「鶴間池です」に対して「で、それをどうしますか?」と返すなら、聞かないほうがましである。そこで **`ask_user` は結果を返す Tool** として設計されている(§3.4.2):

- 中断したターンは、**問い・選択肢・`kind` をスレッド状態 `pending_ask` に残す**(§4.1)
- 次のターンで `load_context` が答えを **`ask_user` の実行結果**に組み立て、`tool_results` として `understand` のプロンプトに載せる
- `understand` は**「何を聞いて、何と答えられたか」を見た状態で** plan を組む。**曖昧さが消えているので、2 回目の導出は 1 回目より易しい**
- **中断した plan や中間結果は保存しない。**状態は毎ターン DB から再構築するという原則(§4.1 / [ADR-0004](../adr/0004-conversation-pipeline.md))を崩さないため
- `pending_ask` は**次のターンで必ず失効する**(ユーザーが別の話を始めた場合も含む)。**古い問いに答えたことにしない**

**過剰に聞くリスクは実在する。**LLM の棄権能力は問題種別に依存し、強い指示を与えると**答えられる問いまで棄権する**方向に振れることが報告されている(COLING 2025。[agent_patterns_survey.md §2.5](agent_patterns_survey.md))。G1〜G9 はその抑制である。**計測テーブルを持たない(NFR-7 削除)ので、聞きすぎているかどうかは実際に使ってみて判断する**ことになる。多いと感じたら G9 の判定を厳しくする。
...
- **`kind:"ask_user"` と `kind:"clarify"` は、`ask_user` Tool の `kind` 引数にそのまま対応する**(2026-08-03 改訂)。UI の扱いが違うので別 kind に保つ — `ask_user` は選好のスロットを埋めるチップ、`clarify` は**曖昧だった表層形をその場で置き換えるチップ**である。**Tool が 1 つに統合されてもイベントは 2 kind のまま**なので、[ADR-0018](../adr/0018-ask-user-resumable-tool.md) はフロントエンドの契約を変えない
...
| 選好キー語彙(12 語) | 80 | `PreferenceKey` |
| **生タグ語彙(80 語)** | **250** | **`static.tag_vocabulary` の全件。2026-08-03 追加(下記)** |
| `recommend.filter` の enum 値 | 40 | **`mobility` の 3 値を明記する。**2026-08-03 追加(下記) |
| ユーザー発話 | 200 | |
| **合計** | **約 6,800** | 出力 800 前後。**十分な余裕がある** |

> **2026-08-03 改訂: 生タグ 80 語をプロンプトに載せることにした。**旧版は「**生タグ 80 語は載せない**」と決めていたが、**これが実害のある行き止まりを生んでいた。**
>
> 設計は一方で「生タグは `recommend.filter.tags` と `constraint.args.target` に使える」(§3.1)と LLM に許しながら、**語彙を見せずに**書かせ、`validate_plan` の事前条件が**実在しないタグを含む手を丸ごと破棄**する(「実在しない生タグです」)。**LLM は見たことのない語彙を当てにいくしかなく、外すと推薦が 1 件も返らない。**
>
> 実測(2026-08-03): 「自然・山」→ `tags:["自然","山"]` で **`自然` は実在するのに `山` のせいで手ごと破棄**。同様に **`recommend.filter.mobility` の enum 値(`avoid_walk`/`short_walk_ok`/`hike_ok`)を Tool 引数の説明に書いていなかった**ため、「車で回ります」から `mobility:"car"` が生成され **P3 で手ごと破棄**された。**「滝と湧水が好きです。車で回ります。おすすめを教えて」という平易な発話で推薦が 0 件になる。**
>
> **予算には余裕がある**(合計 6,800 / 16,384)。**語彙を見せないことで節約できる 250 トークンより、推薦が返らないことの害が大きい。**
>
> 関連: [23_ux_issues.md](../23_ux_issues.md) / **タグを外したときに手を丸ごと破棄するか、そのタグだけ落として実行するかは別の論点**(§23 論点 20)。
...
#### この設計が `ask_user`(§3.4)に効く

**「2 番目のやつ」の多くは履歴だけで一意に決まる。**残るのは、複数のリストが宙に浮いている場合(推薦の 2 番目か旅程の 2 番目か)のように、**文脈が増えても一意に決まらないケース**だけである。つまり **`ask_user` の発火率が下がる。**履歴が第一防衛線、聞くのが最後の砦という関係になる。
...
| **`asked`**(`plan` に `ask_user` があったか。あれば `kind`)| 聞きすぎていないかを目で見て判断する材料。**2026-08-03: 旧 `understand_action` を置き換え**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))|
| **`resumed_from_ask`**(前ターンの `pending_ask` から復帰したか)| 中断・復帰が成立しているかを見る。**答えたのに復帰していないターンは行き止まりの兆候**である |
...
flowchart TB
  ...
  subgraph GRAPH["ワークフロー（ノード）— 順序はコードが固定する"]
    direction TB
    N1["N1 load_context<br/>決定的<br/>※ pending_ask があれば<br/>tool_results を組み立てる"]
    N2["N2 understand<br/>LLM 1 回目・guided JSON"]
  ...
  subgraph TOOLS["Tool — 呼ぶかどうかは LLM の plan が決める"]
    ...
    T5["T5 ask_user<br/>※ 実行するとターンを中断する"]
  end
  ...
  N5 --> N6
  N6 --> DONE(["done イベント"])
  DONE -.->|"E7 pending_ask がある場合<br/>次ターンの N1 が答えを<br/>tool_results として復帰させる"| N1
```
...
- **`E7` はグラフの辺ではない。**ターン境界をまたぐ復帰を示す破線で、実体は「`pending_ask` を DB に置き、次ターンの `N1` が読む」だけである(§3.4.2)。**ターン内にループは無い**
- **2026-08-03: 辺 E6(`understand` → `respond`)は削除された。**聞き返しは `plan: [ask_user]` として通常経路(`N3` → `N4`)を通る([ADR-0018](../adr/0018-ask-user-resumable-tool.md))
...
| N2 | `understand` | **LLM** | **1 回** | 意図 / `plan` / 選好・制約の抽出 / 照応解決を 1 回の guided JSON で出す。**解けないときは `plan: [ask_user]` を出す**(§3.4)。**`tool_results` があればそれを見た状態で走る**(**詳細: [understand_node.md](understand_node.md)**) | 1 回だけ再試行 → 致命 |
...
| T5 | `ask_user` | 同上(P5 により末尾のみ・1 手まで) | `asked_slots` / `ask_streak` / **`pending_ask`** を更新 | **中断する** | なし |

**`ask_user` だけがグラフの制御に影響する Tool** である。実行されると TurnState の `should_end_turn` が立ち、`act` は以降の手を実行しない(P5 によりそもそも後続の手は無い)。ただし**ノードの並びは変わらない** — `respond` は通り、そこで質問文の日本語が書かれる([ADR-0007](../adr/0007-preference-elicitation.md))。つまり「ターンを中断する」とは**Tool の実行が答え待ちで止まる**という意味であって、グラフを途中で抜けることではない。

**そして `ask_user` は結果を返す唯一の「ターンをまたぐ Tool」である**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。答えは次のターンの `tool_results` に入り、`understand` が受け取る(§3.4.2)。
...
| E5 | `should_end_turn` が立った(`ask_user` を実行した) | `act` 内で残りを実行せず → `respond`(質問モード) | 同上 |
| ~~E6~~ | ~~`understand` が `ask_user` を選んだ~~ | — | **2026-08-03 削除**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。聞き返しは `plan: [ask_user]` として通常経路を通る |
| **E7** | **`pending_ask` が残っている**(前ターンで `ask_user` が中断した) | **次ターンの `load_context` が答えを `tool_results` に組み立て、`understand` へ渡す** | **グラフの辺ではない。**ターン境界をまたぐ復帰であり、ターン内にループを作らない |

**グラフの巡回は E1 だけで、しかも上限 1 回。**「LLM が自分で反復する」構造をどこにも持たないことが、この表で確認できる。**E7 を足してもこの性質は変わらない** — 復帰は必ず**ターン境界**で起き、次の観測(ユーザーの答え)は**ターンを終えないと得られない**からである(§3.4.3)。
...
TurnState = {
  ...
  tool_results,                   // 前ターンで中断した ask_user の答え（§3.4.2）。無ければ空
  ...
  should_end_turn,                // ask_user が立てる（= ターンを中断する）
  pending_ask,                    // 中断した問い。persist が threads に書く（§3.4.2）
...
| **質問** | `should_end_turn`(`ask_user` Tool が実行された) | 質問文 1 つと、選択肢の自然な言い換え。**`kind` で書き分ける** — `preference` なら聞きたいこと、**`clarify` なら「何が曖昧だったか」**を述べ、**推測で先に進んだふりをしない** |
| **不成立** | 手が 0(P8)/ 全手が失敗 / `understand` 致命 | 何が分からなかったか + 次の一手の促し |

> **2026-08-03: 4 つ目の「聞き返し」モードは「質問」モードに統合された**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。`ask_user` が 1 つの Tool になり、両者の入力が同じ形になったため。書き分けは `kind` 引数で行う。
...
### 18.7 `ask_user`(2026-08-03 改訂: 結果を返す 1 つの Tool)

```jsonc
in  = {
  kind: "preference" | "clarify",
  slot?:    Slot,       // kind = "preference" のとき必須
  surface?: string,     // kind = "clarify" のとき必須（曖昧だった表層形）
  reason:   string,
  options:  [{label: string, value: string}]    // 2〜4 個（G5）
}

out = {                 // ★ ターンを中断し、次のターンで返る（§3.4.2）
  answer:      string,
  answered_by: "chip" | "free_text",
  slot?:    Slot,
  surface?: string
}
```

- 事前条件: **ガードレール(§5.1)**。P5 により **plan の末尾にしか置けず、1 手まで**
- **1 つの Tool で 2 用途を担う**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。`kind` が **SSE の `state` の kind(`ask_user` / `clarify`)にそのまま対応する**([chat_sse.md §1.2](../40_api/chat_sse.md))
- **実行するとターンを中断する。**`pending_ask` を保存し、次のターンで `load_context` が答えを `tool_results` に組み立てて `understand` へ渡す
- **`pending_ask` は 1 ターンで失効する。**古い問いに答えたことにしないため
- **質問文の日本語を書くのは `respond`。**この Tool は問いと選択肢を確定させるだけ
...
flowchart LR
  ...
  subgraph ASK["ask_user"]
    B1["kind / slot|surface / options"] --> B2{"§5.1 の<br/>ガードレールを<br/>満たすか"}
    B2 -->|"満たさない"| B3["破棄<br/>ログに記録"]
    B2 -->|"満たす"| B4["asked_slots に追加<br/>ask_streak += 1<br/>pending_ask を立てる"]
    B4 --> B5["state: ask_user / clarify<br/>ターンを中断"]
    B5 -.->|"次のターン"| B6["答えを tool_results に<br/>→ understand へ"]
  end
```
...
| `ask_user` | **§5.1 のガードレール(G1〜G9)。`kind` で効き方が変わる** | 破棄しログに記録 |

**2026-08-03 改訂**: 聞き返しも `ask_user` Tool になったので、**事前条件は本表の 1 行に集約された**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))。旧版は「聞き返しは Tool ではないので本表に現れない」と注記していた。
...
| **18** | **`understand` に聞き返しをさせるか**(ユーザー提案) | ~~させる。`ask_user` と `done` の 2 Tool を持つ有界エージェントにする~~ → **2026-08-03 に決定を改めた。**論点 19 を見ること | **§3.4** / §5.1 / [ADR-0010](../adr/0010-understand-bounded-agent.md)(廃止) |
| **19** | **`ask_user` が「呼んで終わり」で返り値を持たないのはおかしい**(2026-08-03、ユーザー指摘) | **`ask_user` を「結果を返す 1 つの Tool」に統合する。**聞き返しも `plan: [ask_user]` の 1 手として表現し、答えは次ターンの `tool_results` として `understand` に返る。**論点 18 の「plan を書かせる矛盾」という前提が誤りだった** — そのときの plan は 1 手でよい | **§3.4** / §5.1 / §15.6(E6 削除・E7 追加)/ §18.7 / [ADR-0018](../adr/0018-ask-user-resumable-tool.md) |
| **20** | **引数の 1 項目が不正なとき、手を丸ごと破棄してよいか**(2026-08-03、実機で発覚) | **未決。**現行は P3 / 事前条件が**手ごと破棄**する。実測では「`自然` は実在するのに `山` のせいで推薦が 0 件」「`mobility:"car"` のせいで推薦が 0 件」が起きた。**§7 の語彙提示で発生頻度は下がるが、外れたときに全滅する性質は残る。**[ADR-0005](../adr/0005-itinerary-solver.md) の「無言破棄の禁止」からすると、**不正な項目だけ落として実行し、落としたことを `unmodeled` で伝える**ほうが筋に見える。**ユーザーの判断を仰ぐ** | §1.3(P3)/ §20.2 / [23_ux_issues.md](../23_ux_issues.md) |
...
**2026-08-03、その実現手段を改めた。**能力そのものは残したまま、`understand` のノード内分岐(ADR-0010)から **`ask_user` Tool への統合**([ADR-0018](../adr/0018-ask-user-resumable-tool.md))に変えた。詳細は §3.4。要点だけ:

- **LLM 呼び出しは増えない**(`understand` 1 + `respond` 1、ターン合計 2〜3 回のまま)
- **ターン内にループを作らない**ので [ADR-0008](../adr/0008-plan-then-execute.md) と矛盾しない。**中断・復帰は必ずターン境界で起きる**
- **Tool が結果を返すようになった。**答えは `tool_results` として `understand` に戻り、**質問を発した文脈が失われない**
- **最大のリスクは「聞きすぎ」。**§5.1 の G1〜G9 で縛る。加えて §7 の会話履歴(直近 3 ターンを生テキストで渡す)が第一防衛線として効く。**計測はしない**ので、多いと感じたら G9 を厳しくする
