...
- **実装補足(2026-08-04)**: vLLM/xgrammar の guided decoding には「**配列要素内の number フィールド直後に空白トークンを無限出力する**」既知不具合がある(gemma-4-31B で実機再現)。そのため本ステップは **guided の 1 回目が壊れたら、guided を外したテキスト指示の 2 回目にフォールバック**する(それでも失敗したら差分なしで縮退)。同構造を持つ他のスキーマの監視は [25_known_issues.md §2-1](../25_known_issues.md)
...
### 3.5 停止条件(決定 2026-08-04、論点 4。R1/R4 改訂 2026-08-06、[ADR-0024](../adr/0024-ask-user-proactive-hitl.md))

| # | 条件 | 挙動 |
| --- | --- | --- |
| R1 | **手数上限 8**(`done` と **`ask_user` を除く**実行手で数える。質問はループ予算を消費しない — 改訂 2026-08-06) | 到達で「まとめに入って」を挿入し、次周は `done` のみ許す縮小スキーマに切り替える |
...
| R4 | **`ask_user` は 1 ターン 6 回まで(メイン・SA 合算。暴走時の安全弁であって UX 目標値ではない — 改訂 2026-08-06)** | 超えたらスキーマから外し、**最も確からしい解釈を採って仮定を明示して進める** |
...
flowchart LR
  ...
  B -->|"ask_user<br/>（推薦を左右するスロットが不明）"| C["UI に質問を提示し回答を待つ（§7）<br/>回答 → update_profile（§2）<br/>→ 回答と更新後プロフィールを<br/>act に持ち帰る"]
  C --> B
  B -->|"done{filter}"| D["既存のレコメンド処理（変更なし）<br/>ハードフィルタ → スコアリング<br/>→ provisional → LLM リランク → final"]
  D --> E["RecommendationResult → メインへ"]
```

- **LLM の役割は 2 つだけ**: (i) プロフィールと指示で推薦が成立するかの判定(**推薦を左右するスロット(同行者・歩行耐性・興味など)が不明なら `ask_user` で聞く** — 改訂 2026-08-06、[ADR-0024](../adr/0024-ask-user-proactive-hitl.md))、(ii) `instruction` → `filter`(タグ・mobility・area・day 等)への翻訳。**タグ 80 語と `mobility` の enum(`avoid_walk` / `short_walk_ok` / `hike_ok` — 歩行耐性であって移動手段ではない)は、このサブエージェントのプロンプトにだけ載せる**
...
- **不明な選好スロットは、ユーザーが選定を任せた場合を除き、聞いてから推薦する**(R4 の枠内で 1 周 1 問。1 回制限は 2026-08-06 に廃止 — [ADR-0024](../adr/0024-ask-user-proactive-hitl.md)。同日実機確認の是正: 「聞いてよい」という許可形のプロンプトでは LLM が 1 問で切り上げ、コールドスタートでも同行者しか聞かなかった — **指示形で書く**)。優先順は **`interests`(タグに直結し推薦結果を最も左右する)→ `party` → `mobility`**。既に分かっているスロットは聞かない(§10 A1/A6)。**ユーザーが選定を任せた場合と質問がタイムアウトした場合だけ**、仮定して推薦し、置いた仮定を結果で報告する(HITL は同一ターン内で回答を持ち帰るため、ターンは必ず推薦で終わる — 質問だけで終わることはない)
- **SA が聞けるスロットは選好 4 種(`party`/`mobility`/`pace`/`interests`)に限定する**(guided schema の enum で強制。2026-08-06 レビュー是正 L-2): `dates`/`origin` はメインエージェントの担当で、SA には起点選択肢の具体値規則(dialogue_style.md 論点 C)を教えていないため聞かせない
- **ガードで弾かれた質問は理由を次周のプロンプトに返し、SA は別のスロットを聞き直せる**(2026-08-06 レビュー是正 M-1: 1 回の拒否で質問能力を止めない)。**同じスロットが 2 度弾かれたら打ち切り**、以後は仮定で推薦する(反復上限と合わせてループは有界)
...
- **譲歩メッセージ(`message_ja`)は発生源から表示名で組む**(2026-08-04 追加、[25 §1-7](../25_known_issues.md)): 譲歩文の唯一の生成点はペナルティレジストリ([recommendation_planning.md §4.4 経路 1](recommendation_planning.md))であり、そこで `target`/`a`/`b` が `spot_id` なら表示名(`name_ja`)へ解決してから文を組む。`message_ja` は旅程ダイジェスト経由で LLM のコンテキストへ、`state:itinerary`・undo/redo・GET 経由でフロントの譲歩カードへ、いずれも無変換で流れるため、発生源で断つのが本則。**過去版として永続化済みの旧形式(spot_id 入り)への防御**として、ダイジェスト整形器と `state:itinerary`/REST の送出層は `message_ja` 中の `spot_` トークンを表示名へ置換してから出す
- **ダイジェストの名前解決が失敗しても `spot_id` へフォールバックしない**(2026-08-04 追加): 旅程ダイジェスト・推薦ダイジェストとも、`spot_names` で名前が引けない要素は中立表記(「(名称未登録の地点)」)にする。id を LLM に見せた時点で応答本文への露出リスクになる(§10 C7)
...
| タイムアウト | **10 分**待って回答がなければ、ツール結果を「**未回答**」としてエージェントに返す。エージェントは仮定を明示して進めるか、`done` でまとめる(質問は `respond` が「保留のまま」と言及する)。**タイムアウトが 1 度起きたターンでは、以後 `ask_user` をスキーマから外す(メイン・SA 共通の `TurnState` フラグ)** — ユーザーが席を外しているとみなし、残りは仮定で進める(ADR-0024 例外 (iii) の実装。2026-08-06 レビュー是正 H-1: これが無いと SA はタイムアウトを観測できず、最大 6 回 × 10 分ターンがブロックされる)。スキーマ除外に加え、**実行時の共通バックストップとして `execute_ask_user` 自身もフラグ成立中の質問を拒否する**(スキーマ再生成を挟まない知識検索 SA の同一呼び出し内ループもこれで止まる) |
...
| # | 規則 |
| --- | --- |
| R1 | 手数上限 8(`done`・`ask_user` を除く実行手 — 改訂 2026-08-06)。到達で縮小スキーマ(`done` のみ)へ |
| R2 | コンテキスト予算 soft 70% / hard 85% |
| R3 | 同一 Tool + 同一引数の反復は実行しない |
| R4 | **`ask_user` は 1 ターン 6 回まで(メイン・SA 合算。暴走時の安全弁 — 改訂 2026-08-06、[ADR-0024](../adr/0024-ask-user-proactive-hitl.md))。**超えたらスキーマから外す |

**`ask_user` の規律(メイン・SA 共通。カウンタは合算。改訂 2026-08-06、[ADR-0024](../adr/0024-ask-user-proactive-hitl.md) — 質問の「量」の抑制をやめ、「質」の規律だけ残す)**

| # | 規則 | 実装 |
| --- | --- | --- |
| A1 | 同じスロットを 2 回聞かない。**適用範囲はスロットの寿命に合わせる**(2026-08-06 レビュー是正 M-3): 選好スロット(`onboarding`/`party`/`mobility`/`pace`/`interests`)は**スレッド生涯**、`dates`/`origin` は**ターン内だけ**(旅程ごとに変わる情報を恒久に封じない) | `asked_slots`(スレッド状態。**永続するのは選好スロットのみ** — `dates`/`origin` はターン内のメモリ上でだけ照合) |
| A2 | **廃止(2026-08-06)**: 旧「質問を含むターンの連続は 2 ターンまで」。不明が残っていれば次ターンでも聞いてよい | `ask_streak` を撤去 |
...
| A6 | **既に分かっていることを聞かない**(プロフィール・会話に値があるスロットの再質問は逆に体験を損なう) | プロンプト + 判定 |
| A7 | 解決できない選択肢を出さない: 送出前に名寄せし、解決不能な選択肢だけ除去。除去後 2 個未満なら A3 で差し戻し([dialogue_style.md 論点 C](dialogue_style.md)。**旧 A7「レコメンド SA の質問は 1 回まで」は 2026-08-06 廃止** — ADR-0024) | `filter_unresolvable_ask_user_options` |

**聞くか、仮定して進めるか — 判断基準は「結果を左右する情報が不明か」**(改訂 2026-08-06、[ADR-0024](../adr/0024-ask-user-proactive-hitl.md)。旧基準「間違いが静かに起きるか」を置き換え): 推測で的外れな旅程・提案を返すほうが、質問の手間より体験を損なう。**推薦・旅程の質を左右する情報(日付・起点・同行者・歩行耐性・興味など)がプロフィール・会話・軌跡のどこからも得られないなら `ask_user` で聞く。**ただし**日付・時間枠を聞くのは旅程を作る・変えるターンだけ**である(2026-08-06 レビュー是正 M-2: 推薦だけのターンでは日付は結果に影響しない — 聞くこと自体がこの判断基準に反する)。仮定して進めてよいのは (i) ユーザーが選定を任せた(「お任せ」「おすすめで」)、(ii) 結果にほとんど影響しない軽微な隙間、(iii) 質問がタイムアウトした・ガード等で実行できなかった(§7。結果にその旨が返る)、の 3 場合だけ(置いた仮定は従来どおり `assumptions` で明示する)。照応の曖昧さ(複数 POI に解け得る・破壊的操作の解釈が割れる)で聞くのは従来どおり。
...
| C7 | **`spot_id` をユーザー可視面に出さない**(2026-08-04 追加、[25 §1-7](../25_known_issues.md)): LLM のコンテキストへ渡る文字列(ダイジェスト・譲歩文・制約表示)とフロントへ送る表示用文字列(`message_ja` 等)は表示名で組む。名前が引けないときも id へフォールバックせず中立表記を使う。`respond` 本文の事後検査(禁止語。[dialogue_style.md §5](dialogue_style.md))はこの規則と独立の最終防御として維持する |
...
| `asked_slots` / `resolved_ambiguities` | スレッド行 | スレッド(`asked_slots` に永続するのは選好スロットのみ — §10 A1。`ask_streak` は 2026-08-06 廃止、ADR-0024・migration 0005) |
...
**一括 commit の例外(2026-08-04、[ADR-0020](../adr/0020-routes-early-commit.md))**: 会話状態の commit は N6 persist の 1 回だけ、が原則だが、**冪等キャッシュ(`app.routes`)と `pending_ask` は専用の短寿命セッションで即時 commit する**。前者は `state:itinerary`(final)送出時点で `GET /routes/{id}` が解決できるため([geo.md §3.5](geo.md))、後者はリロード復元のため(§7)。どちらも会話状態ではなく、ターンの rollback で残っても無害である。
...
Itinerary     = {days: [ItineraryDay], concessions: [Concession], version,
                 assumptions: [string]}   // 未確認の前提（日付・起点等）。版ごとにコピー。2026-08-04 追加
