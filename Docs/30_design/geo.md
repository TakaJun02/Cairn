...
2. **1 レッグ = 起点から終点までの door-to-door の移動 1 本。**内部に mode の違う **1〜3 本のセグメント**を持つ
   - **2026-08-01 改訂**: 当初は「1〜2 本」と書いていたが、**両端とも車で直接着けない地点どうし**のレッグ(例: 法体の滝 → 丸池様)は `foot → car → foot` の **3 セグメント**になる。door-to-door の定義(§4.1 の `walk(i) + drive + walk(j)`)から必然的に導かれる形であり、2 本に丸めると出発側の徒歩が線から消える。`mode_summary` は引き続き `car+foot`
...
各 spot について:
  ...
  2. さもなくば、次の 2 種類を候補にする
       (a) access_points（private を除く）から直線距離の近い順に 5 件
       (b) ★ 1 で得た「車道スナップ点」そのもの（= 車で寄れる最寄りの路上）
     各候補 → 地点の徒歩ルートを OSRM に問い、到達できたもののうち所要が最小のものを採る
       (b) が選ばれたとき: car_node = スナップ点、access_point_id = NULL、
                           direct_by_car = false、walk_* は実測の徒歩ルート
...
- 出力は `direct_by_car` の内訳を必ず標準出力に出す(「43 件中 直接 31 / 駐車場経由 12、最長徒歩 14 分」)。**この括弧内は出力書式の例であって実測値ではない**

#### 2.3.1 候補 (b)「車道スナップ点」を足した理由(**2026-08-01 改訂。実装中に発見**)

**当初のアルゴリズム(候補は access_points だけ)は、実データで破綻した。**`build-geo` を実際に走らせた結果(2026-08-01 実測):

| spot | 車道スナップ距離 | 当初アルゴリズムの結果 |
| --- | --- | --- |
| spot_006 遊佐町総合運動公園 | **54.9 m** | 駐車場経由・徒歩 **158 分** |
| spot_002 赤田の大仏 | **119.1 m** | 駐車場経由・徒歩 **394 分**(32.8 km) |
| spot_018 丸池様 | 123.6 m | 駐車場経由・徒歩 184 分 |
| spot_042 牛渡川 | 289.0 m | 駐車場経由・徒歩 142 分 |
| spot_007 法体の滝 | 308.1 m | 駐車場経由・徒歩 118 分(9.9 km) |

**「50 m を 5 m 超えただけで、30 km 先の駐車場を探しに行く」**という穴があった。原因は 2 つ:

1. **`access_points` は 33 件しかなく、43 地点をカバーしていない。**エリア全体に散在しているので、近くに 1 件も無い地点では「近い順 5 件」がすべて数十 km 先になる
2. **「車道の路肩まで行って少し歩く」という最も普通の接近が、候補に入っていなかった**

**帰結は重い。**`travel_times.car` は door-to-door(§4.1)なので、この徒歩がそのまま行列に乗る。実測で **平均 137 分・最大 657 分** になり、**この行列でソルバーを回すと 1 日に 1〜2 か所しか入らない旅程しか出ない**(FR-2 が成立しない)。

**候補 (b) を足すと、これが実測どおりの値に戻る。**赤田の大仏は「路上に停めて 119 m 歩く」= 約 2 分になる。**鳥海山大物忌神社(山頂)のように本当に数時間歩く地点は、そのまま数時間のまま残る** —— これは捏造ではなく事実であり、[§4.4](#44-欠損を許さない) の「推定値で埋めない」に反しない。

- **閾値を 50 m から広げる案は採らない。**広げると `walk_sec = 0` のまま実際には 300 m 歩くことになり、**時刻が過小評価される**(ADR-0013 が塞いだはずの破れの再来)。**歩く距離は歩く距離として測る**
- **スナップ点は「駐車場」ではない**ので `access_point_id` は NULL にする。`direct_by_car` も false のままにする(地点まで車で行けてはいない)
- **`build-geo` は徒歩時間の分布(中央値・最大・60 分超の件数)を標準出力に出す。**データの穴が数字で見えるようにする(P6)。**60 分超があっても失敗させない** —— 山頂・避難小屋は本当に遠い

**改訂後の実測(2026-08-01)**: 直接 18 / access_point 経由 14 / **車道スナップ経由 11**。徒歩の中央値 0 分・最大 87 分・60 分超は 5 件(竜ヶ原湿原・祓川神社・赤滝・鳥海山大物忌神社・七ツ釜避難小屋 —— **いずれも実際に登山を要する地点**)。`travel_times.car` は **平均 136.8 → 71.6 分・最大 657.4 → 237.3 分**に是正された。

> **既知の近似(2026-08-01 記録)**: 車道スナップ点が選ばれた地点のうち **4 件**(赤田の大仏 119 m・遊佐町総合運動公園 55 m・胴腹滝 96 m・牛渡川 289 m)は、**foot グラフ上で地点と路上点が同じノードにスナップするため `walk_sec = 0` になる**。実際には最大 300 m 弱(徒歩 4 分程度)の接続区間がある。**行列の誤差は 1 地点あたり最大 ±4 分**で、是正前の 394 分に比べれば無視できる大きさなので、**当面この近似を受け入れる**。直すなら「foot ルートが 0 を返したときだけ、地点のスナップ距離を徒歩速度で時間に換算して足す」で足りる。
...
| **18** | **接近の候補に「車道スナップ点」を足す**(§2.3.1。**2026-08-01 実装中に改訂**) | `access_points` 33 件が 43 地点をカバーしておらず、当初案では赤田の大仏の徒歩が **394 分**になった。`travel_times` は door-to-door なのでこれが行列に乗り、**1 日 1〜2 か所しか入らない旅程しか出なくなる** |
| **19** | **スナップ閾値は 50 m のまま広げない** | 広げると `walk_sec = 0` のまま実際には歩くことになり、[ADR-0013](../adr/0013-leg-route-door-to-door.md) が塞いだ「徒歩ぶんが時刻に入らない」破れが再発する |
| **20** | **1 レッグのセグメントは 1〜3 本**(`foot → car → foot` を認める) | 両端とも車で直接着けないレッグで必要になる(§1.2) |
