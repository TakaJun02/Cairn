...
- **`spot_id` をユーザー可視のテキストとして描画しない**(2026-08-04 追加、[25 §1-7](../25_known_issues.md)): スポット名の解決(`navStore.spots`)に失敗したときのフォールバックは `spot_id` ではなく中立表記「不明な地点」にする。候補カード・旅程行・diff 行・パック取り込み・地図ポップアップなど、名前を表示するすべての箇所に適用する。譲歩(`concessions[].message_ja`)はサーバー契約([chat_sse.md §1.2](../40_api/chat_sse.md))で `spot_id` を含まないが、過去版データへの防御として、表示前に `spot_〜` トークンを検出したら表示名(引けなければ「不明な地点」)へ置換する表示フィルタを通す
