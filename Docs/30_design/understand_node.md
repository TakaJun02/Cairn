...
- 状態: **廃止(2026-08-04。Understand ノードそのものが [ADR-0019](../adr/0019-react-main-agent-subagents.md) で廃止された。新設計は [agent_react_architecture.md](agent_react_architecture.md))。**記録として凍結
...
4. **ただし「分からない」と言える。**(2026-07-31 追加 / 2026-08-03 改訂)翻訳できないときは、**`plan` に `ask_user` を 1 手だけ載せて聞ける**。下記。

### 0.1 解けないときは `ask_user` を 1 手だけ載せる(2026-08-03 改訂)

**決定 2026-08-03([ADR-0018](../adr/0018-ask-user-resumable-tool.md)、ユーザー指示)。**`understand` は翻訳しきれないとき、**`plan` に `ask_user` を 1 手だけ載せる。**

```mermaid
flowchart LR
  A["発話<br/>+ tool_results（あれば）"] --> B["understand<br/>LLM 1 回"]
  B --> C["抽出できるものを埋める<br/>参照 / 選好 / 制約 / unmodeled"]
  C --> D{"翻訳しきれたか"}
  D -->|"解けた"| E["intent + plan を出す<br/>→ 以降のノードへ"]
  D -->|"解けない"| F["plan: [ask_user] を 1 手<br/>→ 同じく validate_plan → act へ"]
```

> **2026-08-03 改訂。**旧版は [ADR-0010](../adr/0010-understand-bounded-agent.md) に基づき、`understand` が **`done` / `ask_user` の 2 択**を持ち、`ask_user` を選んだら `plan` を出さずに `respond` へ抜ける(辺 E6)設計だった。**`ask_user` は「結果を返す 1 つの Tool」に統合され、`action` と `clarify` フィールドも E6 も廃止された。**

**なぜこれが要るのか。**「2 番目のやつを外して」で候補が 2 つあるとき、従来は**「賭ける」か「破棄して謝る」**の 2 択しかなかった。賭けて外すと**気づかないまま違う POI が旅程から消える**(§2 の `references` の行)。**聞くことは 3 つ目の選択肢**である。

**押さえるべき制約が 4 つある。**

- **LLM 呼び出しは増えない。**聞くターンも `understand` 1 回 + `respond` 1 回の計 2 回
- **ターン内でループしない。**次の情報(ユーザーの答え)はターンを終えないと得られないので、**中断は必ずターン境界で起きる**
- **`ask_user` は結果を返す。**答えは次のターンの **`tool_results`** としてこのノードのプロンプトに載る。**質問を発した文脈が捨てられない**([agent_planning_phase.md §3.4.2](agent_planning_phase.md))
- **選択肢を 2〜4 個「具体的に」出せないなら聞けない**(ガードレール G6)。`spot_id` か既定の解釈 enum に解決できることをコードが検査する。**聞いても解けない曖昧さを構造的に弾くための条件**である

判断基準は「**間違いが静かに起きるか**」。照応の取り違えは聞く。日付の不明は聞かずに仮定して見せる(間違っても画面に出るので直せる)。詳細は [agent_planning_phase.md §5.1](agent_planning_phase.md)。
...
| **`ask_user` の `options` が 2〜4 個ある** | **保証する**(2026-08-03 追加) | — |
...
> **2026-08-03: `plan[].args` の一部を guided decoding で縛ることにした。**旧版は `args` を無制約(`additionalProperties: true`)にし、中身はすべて `validate_plan` が検証していた。ところが **LLM が `ask_user` を `options: []` で出し、G5 が手ごと破棄する**ため、**「聞きたい」と判断しているのに質問そのものが消える**という行き止まりが実機で再現した([23_ux_issues.md §1-1](../23_ux_issues.md))。
>
> **落として検出するより、出せなくするほうが確実である**(§3.2 の `spot_id` を enum で縛るのと同じ原則)。したがって **`ask_user` の引数だけは guided decoding のスキーマに載せ、`options` に `minItems: 2` / `maxItems: 4` を課す。**他の Tool の引数は従来どおり `validate_plan` が検証する。
...
> **`spot_id` 以外の語彙も「見せてから書かせる」**(2026-08-03 追加)。**生タグ 80 語**(`static.tag_vocabulary`)と **`recommend.filter.mobility` の 3 値**は、コードが実在検証するのに**プロンプトに載っていなかった。**その結果 LLM は見たことのない語彙を当てにいき、外すと `validate_plan` が**手を丸ごと破棄**して推薦が 0 件になる。**検証する語彙は必ず見せる**([agent_planning_phase.md §7](agent_planning_phase.md) の改訂)。`spot_id` を絞る理由(**指せないものを構造的に指せなくする**)と同じ原則の裏返しである。
