...
**「実装が実際にどうなっているか」を先に掴みたいなら [24_architecture_as_built.md](24_architecture_as_built.md)。**バックエンド全体と旅程計画エージェントを、実装と照合したうえで解説してある。
...
| 2 | [30_design/agent_react_architecture.md](30_design/agent_react_architecture.md) | **対話エージェントの全設計(ReAct 構成。決定稿 2026-08-04)**。§14 が**型と enum の正**。旧 `agent_planning_phase.md` / `understand_node.md` は**廃止(凍結)** |
...
| 事柄 | どこが正か |
| --- | --- |
| 型・enum(`PredEnum` / `PreferenceKey` / `Slot` / `ToolErrorCode` ほか) | **[agent_react_architecture.md §14](30_design/agent_react_architecture.md)** |
...
| [0008](adr/0008-plan-then-execute.md) | ~~一括プラン方式(ReAct を採らない)~~ (**廃止: ADR-0019**) |
| [0009](adr/0009-no-subagents.md) | ~~サブエージェントを持たない~~ (**廃止: ADR-0019**) |
| [0010](adr/0010-understand-bounded-agent.md) | ~~`understand` は 2 Tool の有界エージェント~~ (**廃止: ADR-0018**) |
...
| `23_ux_issues.md` | 旧・一括プラン方式の系に対する UX 問題インベントリ(**凍結**。現状は 25 が正) | 更新しない(記録) |
| `24_architecture_as_built.md` | **実装されたアーキテクチャの解説**(ReAct 構成。**2026-08-04 に実装と照合済み**、食い違いは §7) | 実装と文書を照合したとき |
| `25_known_issues.md` | **現時点で残っている問題の一覧(正)。**23 の各項目の現状追跡 + 再構築後の新規項目・監視項目 | 実機確認・問題の解消のたび |
